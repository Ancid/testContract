...
describe("Givers Chain tests", function () {
  describe("Deployment", function() {
    beforeEach(async function() {
      ...
      const GiversToken = await ethers.getContractFactory("GiversChain");
      this.hardhatToken = await GiversToken.deploy(addr1.getAddress(), addr2.getAddress(), process.env.ROUTER02);
    ...
    it("Should set the right supply amount", async function() {
      ...
      expect(await this.hardhatToken.totalSupply()).to.equal(expectedTotalSupply)
    })

    it("Should assign the total supply of tokens to the owner", async function() {
      this.timeout(100000)
      const totalSupply = await this.hardhatToken.totalSupply();
      expect(await this.hardhatToken.balanceOf(this.owner.address)).equal(totalSupply);
    });
  });
  ...
  describe("", function() {
    ...
    describe("Liquidity", function() {
      it("Should add right amount of liquidty", async function() {
        ...
        const pairAddress = hardhatToken.uniswapV2Pair()
        this.pair = new ethers.Contract(
          pairAddress,
          ['function totalSupply() external view returns (uint)', 'function balanceOf(address owner) external view returns (uint)', 'function approve(address spender, uint value) external returns (bool)', 'function decimals() external pure returns (uint8)', 'function getReserves() external view returns (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast)'],
          this.provider
        )
        const pairsigner = this.pair.connect(owner)
        const Ownerliquidity = await pairsigner.balanceOf(owner.address)
        const totalLiquidity = await pairsigner.totalSupply()
        const decimals = await pairsigner.decimals()

        const reserves = await pairsigner.getReserves()
